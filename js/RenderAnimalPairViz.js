export function renderAnimalPairViz(containerId="#animal-pairs-viz") {
    ...
    const svg = c.append("svg").attr("width","100%").attr("height","100%").style("background","#f5f5f5")
    const g = svg.append("g").attr("transform",`translate(${m.left},${m.top})`)

    const caption = c.append("div")
        .style("position","absolute")
        .style("left","50%").style("top","70%")
    ...
    g.append("image").attr("href","../images/catprod.png")
        .attr("x",cxCat-img/2).attr("y",cy-img/2)
        .attr("width",img).attr("height",img)
        .style("pointer-events","none")
    g.append("image").attr("href","../images/dogprod.png")
        .attr("x",cxDog-img/2).attr("y",cy-img/2)
        .attr("width",img).attr("height",img)
        .style("pointer-events","none")

    const ang=d3.range(-Math.PI/2,Math.PI/2+1e-6,Math.PI/(N-1))
    const sides=[{k:"cat",cx:cxCat,dir:-1,base:"#f4a261",sel:"#d67227"},
        {k:"dog",cx:cxDog,dir:1,base:"#457b9d",sel:"#2f5c7b"}]

    const pts=sides.flatMap(s=>ang.map((a,i)=>{
        const r=SR+(i%2?-R:R)
        return{id:i,side:s.k,label:cats[i].name,val:cats[i][s.k],
            cx:s.cx+s.dir*Math.cos(a)*r,cy:cy+Math.sin(a)*r,base:s.base,sel:s.sel}
    }))

    const fmt = d3.format(",")
    const picked=new Set(); let catSum=0,dogSum=0
    function updateCap(){
    ...
    const circles=g.selectAll("circle").data(pts).enter().append("circle")
        .attr("cx",d=>d.cx).attr("cy",d=>d.cy).attr("r",R)
        .attr("fill",d=>d.base).attr("opacity",.96).style("cursor","pointer")
        .on("mousemove",(e,d)=>{
            const box=c.node().getBoundingClientRect()
            const nm=d.label.length>24?d.label.slice(0,23)+"…":d.label
            tooltip.style("left",e.clientX-box.left+10+"px").style("top",e.clientY-box.top+10+"px")
                .style("opacity",1)
                .html(`<b>${nm}</b><div>Cat: ${fmt(cats[d.id].cat)}</div><div>Dog: ${fmt(cats[d.id].dog)}</div>`)
        })
        .on("mouseleave",()=>tooltip.style("opacity",0))
        .on("click",(_,d)=>{
            const i=d.id, pair=g.selectAll("circle").filter(p=>p.id===i)
            if(picked.has(i)){
                picked.delete(i); catSum-=cats[i].cat; dogSum-=cats[i].dog
                pair.attr("stroke","none").attr("fill",p=>p.base)
            }else{
                picked.add(i); catSum+=cats[i].cat; dogSum+=cats[i].dog
                pair.attr("stroke","black").attr("stroke-width",2).attr("fill",p=>p.sel)
            }
            updateCap()
        })

    g.selectAll("text.lbl").data(pts).enter().append("text")
        .attr("x",d=>d.cx).attr("y",d=>d.cy+5)
    ...
    clearBtn.on("click",()=>{
        picked.clear()
        catSum=0; dogSum=0
        circles.attr("stroke","none").attr("fill",d=>d.base)
        updateCap()
    })
}
